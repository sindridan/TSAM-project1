...
int main(int argc, char const *argv[]) {
    ...
    do{
        ...
        int sendResults = send(sock, inp.c_str(), inp.size() + 1, 0); //  +1 bc of the trailing 0
        if (sendResults == -1) {
            cout << "Cant send to server!" << endl;
            continue;
        }
        memset(buff, 0, 1025);
        int bytesReceived = recv(sock, buff, 1025, 0);
        if (bytesReceived == -1)
        {
            cout << "There was an error getting response from server\r\n";
        }
        else {
            cout << "From SERVER: " << string(buff, bytesReceived) << endl;
        }
